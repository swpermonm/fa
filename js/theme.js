            ...
            if (!(ios)) {
                $('.main-nav', target).on('mouseenter loadimg', '.sub-nav.has-img .sub-nav-item-link', function() {
                    ...
                    if (typeof $(this).data('img') != 'undefined') {
                        $imgCont.append('<img src="' + $(this).data('img') + '+"/>');
                    }
                });
            } else {
        ...
        $(document).on('change', '#cart-summary .cart-summary-item input', function() {
            ...
            $.post('/cart/update.js', {
                ...
                $('#cart-summary .cart-summary-item input').filter(function() {
                    return $(this).val() == 0
                }).closest('.cart-summary-item').animate({
                    opacity: 0
                }, 250, function() {
                    $(this).remove()
                });
            ...
            if ($(window).height() >= 580 && $(window).width() >= 768) {
                ...
                if ($prodPhotoCont.find('img:not(.zoomImg)').length == 1) {
                    ...
                    var $galleryCont = $('<div id="gallery-cont"/>').append(
                        $prodPhotoCont.find('.thumbnails a').clone().attr({
                            rel: 'gallery',
                            title: ''
                        })
                    ).hide().appendTo('body');
